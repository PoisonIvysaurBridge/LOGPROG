...
############
# Task: INITIALIZATION of Menu Prices, Initial Invetory, End Day Variables & Main Menu Display
# Description: housekeeping of POS variables--Built-in Menu items and their respective prices, Initial Inventory Stock of 0, End Day initializations--and also displays the Main Menu
# Variables Used: cont, Option, Beef, Chix, regFries, LFries, regSoda, LSoda, regBeefC, LBeefC, regChixC, LChixC, IBB, IBP, ICP, IRF, ILF, IBW, IRSC, ILSC customer,totalSale, cntA to cntJ
...
############
# Task: Setup
# Description: allows user to edit price of items in menu then displays menu summary & go back to main menu option
...
# Variables Used: cont, Option, ReturnSD, SD, IBB, IBP, ICP, IRF, ILF, IBW, IRSC, ILSC,curIBB, curIBP, curICP, curIRF, curILF, curIBW, curIRSC, curILSC
...
# Description: 
# Variables Used: cont,Option,ReturnSD,SD, accept, more, strOrder, Qty, IBB,IBP,ICP,IRF,ILF,IBW,IRSC,ILSC, totalSale,customer,Total, amtPaid, cntA to cntJ, sumA to sumJ, QtyA to QtyJ
# Outputs: 
############
            elif SD==3:
                print('\n============ ACCEPT ORDER ============')
                ...
                while accept: # new customer
                    ...
                    while more: # more order
                        ...
                        ####################### x or X to exit accept order and proceed with generation of receipt #######################################
                    ...
                    if Total !=0:   
                        ...
                        amtPaid=float(input('\nAmount received:\t\tP'))
                        if amtPaid < Total:
                            print("lacking: \t\t\t-P"+ str(Total-amtPaid))
                            print("\nPlease pay the lacking amount :)")
                            amtPaid2=float(input('\nAmount received:\t\tP'))
                            print("Change: \t\t\tP"+str(amtPaid2-(Total-amtPaid)))
                        else:
...
############
# Task: Start Day--End Day & Exit Option
# Description: this option ends the day; At the end of the day the system will generate a report that includes the ff: A)total customers for the day B) Sales breakdown C) Total Sales D) Inventory at the end of the day then the system will display the Main Menu; when the Exit option is chosen, the system will no longer go back to the Main Menu.
# Variables Used: cont,Option,ReturnSD,SD, IBB,IBP,ICP,IRF,ILF,IBW,IRSC,ILSC, totalSale, cntA to cntJ, Beef, Chix, regFries, LFries, regSoda, LSoda, regBeefC, LBeefC, regChixC, LChixC
# Outputs: Day-end Report (total customers,sales breakdown and the no. of orders for each item in the menu, total sales, inventory at the end of the day)
############                        
...
import time # I did this for extended cmd viewing
time.sleep(999)
